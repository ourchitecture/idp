...
[ADR-0001](../decisions/0001-intent-driven-architecture.md):
...
- **Layer 3 — Implementations**: the actual stacks in `src/stacks/`
...
```bash
IDP_STACK_PATH="src/stacks/nodejs/react-fastify/rest" \
...
### `src/stacks/go/net-http/rest` — Default and canonical reference (Tier 1)
...
- **Stable binary paths**: avoids ephemeral `go run` temp executables for
  Windows compatibility (see [ADR-0006](../decisions/0006-cross-platform-local-runtime-ux-baseline.md))
...
### `src/stacks/nodejs/react-fastify/rest` — React reference (Tier 1)
...
Stack paths follow the convention `src/stacks/<language>/<framework>/<interface>`.
For example:

```text
src/stacks/rust/axum/rest
src/stacks/nodejs/express/rest
src/stacks/go/chi/rest
```
...
check-contract:
    ...
    IDP_WEB_URL="http://127.0.0.1:$(WEB_TEST_PORT)" \
      ...
      IDP_STACK_PATH="src/stacks/<language>/<framework>/rest" \
...
[ADR-0002](../decisions/0002-stack-layout-and-make-contract.md) for the full
...
IDP_STACK_PATH="src/stacks/<language>/<framework>/rest" \
  moon run contract-tests:check-contract
# or: IDP_STACK_PATH="src/stacks/<language>/<framework>/rest" npm run test:contract
...
```json
{
  ...
  "profiles": ["core", "operational"],
  "stackPath": "src/stacks/go/net-http/rest"
}
```
...
- [tests/contract/README.md](../../tests/contract/README.md) — Quick reference for env vars and run commands
- [src/stacks/README.md](../../src/stacks/README.md) — Stack layout conventions and portfolio roles
- [ADR-0001](../decisions/0001-intent-driven-architecture.md) — Three-layer intent/contract/implementation architecture
- [ADR-0002](../decisions/0002-stack-layout-and-make-contract.md) — Stack layout and GNU Make target contract
- [ADR-0003](../decisions/0003-contract-harness-and-runtime-port-contract.md) — Contract harness design and port contract
- [ADR-0004](../decisions/0004-implementation-portfolio-and-support-tiers.md) — Implementation portfolio and support tiers
- [ADR-0005](../decisions/0005-shared-capability-contract-and-conformance-profiles.md) — Capability contract and conformance profiles
- [ADR-0006](../decisions/0006-cross-platform-local-runtime-ux-baseline.md) — Cross-platform runtime UX baseline
- [ADR-0007](../decisions/0007-moon-required-proto-enhanced-toolchain-policy.md) — Moon orchestration and toolchain policy
