...
export const LIST_FLOW_INSIGHTS_TOOL_DESCRIPTION =
  "List provider-neutral flow insights with optional filters for provider, repo, team, service, actor, or audience.";

export const GET_FLOW_INSIGHT_TOOL_NAME = "get_flow_insight";
export const GET_FLOW_INSIGHT_TOOL_DESCRIPTION =
  "Get a detailed flow insight by ID. The response matches the HTTP flow insights detail contract.";

export const LIST_MY_BLOCKERS_TOOL_NAME = "list_my_blockers";
export const LIST_MY_BLOCKERS_TOOL_DESCRIPTION =
  "List the flow insight signals that are blocking the specified actor (for example, blocked on review or missing evidence).";

export const LIST_SERVICE_RISK_SIGNALS_TOOL_NAME = "list_service_risk_signals";
export const LIST_SERVICE_RISK_SIGNALS_TOOL_DESCRIPTION =
  "List aggregated risk signals for a specific service scope.";
...
async function fetchList(filters: Partial<z.infer<typeof listFlowInsightsInputSchema>>) {
  ...
  return listResponseSchema.parse(JSON.parse(body) as unknown);
}
...
export async function runListFlowInsightsTool(
  rawArgs?: unknown
): Promise<ToolResult> {
...
export async function runGetFlowInsightTool(
  rawArgs?: unknown
): Promise<ToolResult> {
...
export async function runListMyBlockersTool(
  rawArgs?: unknown
): Promise<ToolResult> {
...
export async function runListServiceRiskSignalsTool(
  rawArgs?: unknown
): Promise<ToolResult> {
...
export function registerFlowInsightsTools(server: McpServer): void {
  server.tool(
    LIST_FLOW_INSIGHTS_TOOL_NAME,
    LIST_FLOW_INSIGHTS_TOOL_DESCRIPTION,
    listFlowInsightsInputSchema.shape,
    async (args) => runListFlowInsightsTool(args)
  );

  server.tool(
    GET_FLOW_INSIGHT_TOOL_NAME,
    GET_FLOW_INSIGHT_TOOL_DESCRIPTION,
    getFlowInsightInputSchema.shape,
    async (args) => runGetFlowInsightTool(args)
  );

  server.tool(
    LIST_MY_BLOCKERS_TOOL_NAME,
    LIST_MY_BLOCKERS_TOOL_DESCRIPTION,
    listMyBlockersInputSchema.shape,
    async (args) => runListMyBlockersTool(args)
  );

  server.tool(
    LIST_SERVICE_RISK_SIGNALS_TOOL_NAME,
    LIST_SERVICE_RISK_SIGNALS_TOOL_DESCRIPTION,
    listServiceRiskSignalsInputSchema.shape,
    async (args) => runListServiceRiskSignalsTool(args)
  );
}
